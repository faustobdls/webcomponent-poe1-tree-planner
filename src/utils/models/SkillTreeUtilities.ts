...
export class SkillTreeUtilities {
  ...
  constructor(
    ...
    contextComapre: SkillTreeData | undefined,
    edit: boolean
  ) {
    ...
    SkillTreeEvents.controls.on(
      'ascendancy-class-change',
      this.changeAscendancyClass
    );
    SkillTreeEvents.controls.on(
      'wildwood-ascendancy-class-change',
      this.changeWildwoodAscendancyClass
    );
  ...
  public decodeURL = (hash: string) => {
    ...
    try {
      ...
      for (const node of def.Nodes) {
        this.skillTreeData.addStateById(
          `${node.skill}`,
          SkillNodeStates.Active
        );
      }
  ...
  private encodeURL = () => {
    ...
    this.lastHash = `#${this.skillTreeCodec.encodeURL(
      this.skillTreeData
    )}`;
  };
  ...
  private broadcastSkillCounts = () => {
    ...
    SkillTreeEvents.skill_tree.fire(
  ...
  public changeWildwoodAscendancyClass = (
    start: number,
    encode = true,
    newStart = false
  ) => {
    if (newStart)
      SkillTreeEvents.skill_tree.fire('wildwood-ascendancy-class-change');
  ...
  private changeAscendancyClassInternal = (
    isWildwood: boolean,
    start: number,
    ascClasses: IAscendancyClassV7[],
    encode: boolean
  ) => {
    ...
    if (str !== undefined && str.length !== 0) {
      ...
      for (const id in this.skillTreeData.nodes) {
        ...
        if (
          ...
          node.stats.find((stat) => stat.match(regex) !== null) !== undefined
        ) {
  ...
  private getShortestPath = (target: SkillNode): Array<SkillNode> => {
    ...
    while (frontier.length > 0) {
      ...
      for (const id of current.out) {
        ...
        if (
          (current.ascendancyName === '' &&
            out.ascendancyName !== '' &&
            !out.is(SkillNodeStates.Active)) ||
          (current.ascendancyName !== '' &&
            out.ascendancyName === '' &&
            !current.is(SkillNodeStates.Active))
        ) {
          continue;
        }
  ...
  private getRefundNodes = (source: SkillNode): Array<SkillNode> => {
    ...
    for (const id in this.skillTreeData.classStartNodes) {
      const node = this.skillTreeData.nodes[id];
      if (
        node.is(SkillNodeStates.Active) &&
        node.classStartIndex !== undefined
      ) {
      ...
      for (const node of frontier) {
        for (const id of node.out) {
          ...
          if (out.isMultipleChoiceOption && source.isMultipleChoiceOption) {
            const outchoice = out.in.find(
              (id) => this.skillTreeData.nodes[id].isMultipleChoice
            );
            if (
              outchoice !== undefined &&
              outchoice ===
    ...
    for (const id in skilledNodes) {
      if (
        reachable[id] === undefined &&
        this.skillTreeData.nodes[id].classStartIndex === undefined
      ) {
  ...
  private isSameAscendancyClass = (
    ...
    if (
      this.skillTreeData.isWildwoodAscendancyClass(left) !==
      this.skillTreeData.isWildwoodAscendancyClass(right)
    ) {
      return true;
    }
  ...
  private getAdjacentNodes = (start: {
    [id: string]: SkillNode;
  }): { [id: string]: SkillNode } => {
