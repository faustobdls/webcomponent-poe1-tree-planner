...
export class PIXISkillTreeRenderer extends BaseSkillTreeRenderer {
  ...
  constructor(container: HTMLElement, skillTreeData: SkillTreeData, skillTreeDataCompare: SkillTreeData | undefined) {
    ...
    let replaceElements: (Node | string)[] = [this.pixi.view];
    container.replaceChildren(...replaceElements);
    ...
    const zoomPercent = this.skillTreeData.imageZoomLevels.length > 2 ? this.skillTreeData.imageZoomLevels[1] - this.skillTreeData.imageZoomLevels[0] : 0.1;
    ...
    if (skillTreeData.tree === 'Atlas') {
      this.viewport.moveCenter(this.viewport.center.x * (zoomPercent / 2), this.viewport.center.y * (zoomPercent / 2) - 2000);
    }
    ...
    window.onresize = () => {
      this.pixi.renderer.resize(container.clientWidth, container.clientHeight);
  ...
  public PositionedOn(): void {
    const startNonAscendancyNodesEntries = Object.entries(this.skillTreeData.classStartNodes).map(([_, node]) => _);
    
    const activeNonAscendancyNodesEntries = Object.entries(this.skillTreeData.getNodes(SkillNodeStates.Active));
    const activeNonAscendancyNodeArray = activeNonAscendancyNodesEntries.filter(([_, node]) => node.ascendancyName === '' && startNonAscendancyNodesEntries.includes(_));
    if (activeNonAscendancyNodeArray[0][1]) {
        const activeNonAscendancyNode = activeNonAscendancyNodeArray[0][1];
        this.viewport.moveCenter(activeNonAscendancyNode.x, activeNonAscendancyNode.y);
    }
  }
  ...
  private LoadAssets = (data: (SkillTreeData | undefined)[]): Promise<boolean> => {
        ...
        if (filename === 'LOAD_COORDS') {
          for (const coord in sheet.coords) {
            if (addedAssets.indexOf(coord) < 0) {
              ...
              Assets.add({
                alias: coord,
                src: `${utils.SKILL_TREES_URI}/${i.patch}/assets/${coord}.png`,
              });
            }
          }
        } else {
          if (addedAssets.indexOf(filename) < 0) {
            ...
            Assets.add({
              alias: filename,
              src: `${utils.SKILL_TREES_URI}/${i.patch}/assets/${filename}`,
            });
          }
        }
      }
    ...
    return Assets.load(addedAssets, (progress: number) => {
      ...
      if (newText !== progressText) {
        ...
        loadbar.moveTo(0, 0);
        loadbar.beginFill(0xffffff, 0.75);
  ...
  protected DrawSpriteSheetAssets = (layer: RenderLayer, assets: ISpriteSheetAsset[]): { width: number; height: number }[] => {
    ...
    for (var asset of assets) {
      ...
      const offset = asset.offsetX === undefined ? 0.5 : asset.offsetX;
      ...
      sizes.push({
        width: sprite.width,
        height: sprite.height * (asset.half ? 2 : 1),
      });
    }
  ...
  protected DrawBackgroundAsset = (layer: RenderLayer, asset: ISpriteSheetAsset): void => {
    ...
    if (asset.icon === 'AtlasPassiveBackground') {
      ...
      backgroundSprite.anchor.set(0.506, 0.931);
      ...
      for (let row = 0; row < rows; row++) {
        for (let column = 0; column < columns; column++) {
          ...
          backgroundSprite = PIXI.TilingSprite.from(texture.baseTexture, {
            width: cellWidth,
            height: cellHeight,
          });
  ...
  private DrawArcConnection = (connection: IConnnection): PIXI.Container | null => {
    ...
    if (node.nodeGroup === undefined) return null;
  ...
  private DrawLineConnection = (connection: IConnnection): PIXI.Container | null => {
    ...
    for (let i = 0; i < linesNeeded; ++i) {
      const lineTexure =
        length >= texture.frame.width ? texture : new PIXI.Texture(texture.baseTexture, new PIXI.Rectangle(texture.frame.x, texture.frame.y, length, texture.frame.height));
  ...
  protected DrawHighlights = (layer: RenderLayer, highlights: IHighlight[]): void => {
    ...
    for (var highlight of highlights) {
      ...
      graphic.drawCircle(0, 0, Math.max(size.width, size.height) * 0.85);
      graphic.endFill();
  ...
  protected RenderTooltip = (hovered: SkillNode): void => {
    ...
    if (this.skillTreeData.patch === hovered.patch) {
      ...
      tooltip.beginFill(0x000000, 0.75);
    ...
    if (this.skillTreeDataCompare !== undefined) {
      ...
      for (const id in nodes) {
        ...
        tooltipCompare.beginFill(0x000000, 0.75);
  ...
  private CreateTooltip = (node: SkillNode) => {
    ...
    if (tooltip === undefined) {
      let title: PIXI.Text | null =
        node.name.length > 0
          ? new PIXI.Text(`${node.name} [${node.id}]`, {
              fill: 0xffffff,
              fontSize: 18,
            })
          : null;
      let stats: PIXI.Text | null =
        node.stats.filter(utils.NotNullOrWhiteSpace).length > 0
          ? new PIXI.Text(`\n${node.stats.filter(utils.NotNullOrWhiteSpace).join('\n')}`, { fill: 0xffffff, fontSize: 14 })
          : null;
      let flavour: PIXI.Text | null =
        node.flavourText.filter(utils.NotNullOrWhiteSpace).length > 0
          ? new PIXI.Text(`\n${node.flavourText.filter(utils.NotNullOrWhiteSpace).join('\n')}`, { fill: 0xaf6025, fontSize: 14 })
          : null;
      let reminder: PIXI.Text | null =
        node.reminderText.filter(utils.NotNullOrWhiteSpace).length > 0
          ? new PIXI.Text(`\n${node.reminderText.filter(utils.NotNullOrWhiteSpace).join('\n')}`, { fill: 0x808080, fontSize: 14 })
          : null;
