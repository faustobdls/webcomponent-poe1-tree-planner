...
export abstract class BaseSkillTreeRenderer implements ISkillTreeRenderer {
  ...
  public CustomStartRenderHover(hovered: SkillNode): void {}
  public CustomStopRenderHover(hovered: SkillNode): void {}
  ...
  private DrawAscendancyBackgrounds = (): void => {
    for (const id in this.skillTreeData.ascendancyNodes) {
      ...
      this.DrawAscendancyBackground(
        this.skillTreeData.classes.flatMap(x => x.ascendancies),
        group,
        ascendancyName,
  ...
  private DrawConnectionsForNodes = (layer: RenderLayer, nodes: { [id: string]: SkillNode }): void => {
    ...
    for (const id in nodes) {
      ...
      for (const other of others) {
        ...
        const style = node.group === other.group && node.orbit === other.orbit ? ConnectionStyle.Arc : ConnectionStyle.Line;
        const asset = style === ConnectionStyle.Arc ? `Orbit${node.orbit}${connectionType}` : `LineConnector${connectionType}`;
  ...
  private IsValidConnection = (node: SkillNode, other: SkillNode): boolean => {
    ...
    if (
      (node.is(SkillNodeStates.Pathing) || node.is(SkillNodeStates.Hovered)) &&
      !(other.is(SkillNodeStates.Pathing) || other.is(SkillNodeStates.Hovered) || other.is(SkillNodeStates.Active))
    ) {
      return false;
    }
  ...
  public RenderActiveAscendancy = (): void => {
    if (!this.Initialized) {
      return;
    }
    ...
    this.DrawNodes(RenderLayer.SkillIconsActive, Object.fromEntries(activeAscendancyNodes), {}, { outFrames: true });
  };
