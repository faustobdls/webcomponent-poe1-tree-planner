...
export class PIXIAscendancyTreeRenderer extends BaseSkillTreeRenderer {
  ...
  constructor(container: HTMLElement, skillTreeData: SkillTreeData, skillTreeDataCompare: SkillTreeData | undefined) {
    ...
    const defaultZoomLimits = {
      minWidth: this.skillTreeData.width * (zoomPercent / 4),
      minHeight: this.skillTreeData.height * (zoomPercent / 4),
  ...
  CustomStartRenderHover = (hovered: SkillNode): void => {
    if (!this.Initialized) {
      return;
    }
